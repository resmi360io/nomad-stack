    ...
    providers: [
      {
        ...
        available: true,
        notes:
          'Still the cheapest route from client dollars to baht, and still the one changing under your feet. Wise now serves personal customers with a Thai registered address through a locally incorporated entity licensed by the Bank of Thailand. What is reported consistently, including in summaries of Wise\'s own help article that we could not open directly, is this: after the move, a non-baht payment arriving from someone else into your foreign currency receiving details, including your US routing number or a UK IBAN, is converted to baht, added to your THB balance, and charged a conversion fee. Money you add yourself from your own overseas bank account into a foreign currency balance is described as working as before, so the forced conversion targets exactly the case this page is about, which is a client paying you. The dates are not really in dispute, they have moved. Accounts opened after 21 January 2026 were scheduled to move onto the new rules progressively and to be on them by the end of August 2026. That date has passed, and as of mid-September 2026 we could not confirm the wave completed on schedule, or find anyone describing the forced conversion happening on their own account. If you signed up this year, work on the assumption that you are already on the new rules. The wider programme is separately described as running from March to October 2026 across Thai-address personal customers, which covers both signup groups rather than contradicting the August date. Accounts opened before 21 January 2026 were due to move on 3 August 2026, and Wise is reported to have pushed that back at the end of July to October 2026, with verification documents requested in September. We could not open wise.com from our side to confirm any of it first hand, so check your own account rather than our dates. Two consequences people miss: moving money back out of baht is reported to carry daily caps, and once your balance is baht, paying out to a non-baht account means two conversions instead of one. One wrinkle worth checking yourself: the move is keyed to the registered address on your account rather than your nationality, so a foreign resident who still has a home country address on file may not be in the first wave. Look at the address in your Wise profile rather than assuming. Reports also say a DTV or ED visa is accepted for the Thai verification, which is more than the major Thai banks will accept for opening an account. PromptPay support is part of the new Thai setup, so a converted balance can land on a phone number. We could not confirm from Wise when that went live.',
      },
      {
        slug: 'bank-wire',
  ...
  // USD to Mexico (MXN)
  {
    ...
    providers: [
      ...
      },
      {
        ...
        available: true,
        notes:
          'It carries our best value badge on this corridor, and we want to explain why rather than let you assume it is a recommendation. The badge goes to the cheapest option whose exchange rate margin we can actually source. Western Union wins it here largely by attrition: the three providers a freelancer would more likely want, Higlobe, GrabrFi and Revolut Mexico, are all absent from the priced table because none of them publishes a spread we could read. That is a statement about what we can verify, not about what suits you. Western Union is built and priced for someone sending money to family, not for a client settling a commercial invoice, which is the same mismatch we flag on the Pakistan page. How your transfer gets classified is your client\'s decision rather than yours, and the sender picks the fee and the rate. Fine occasionally. Not a rail to build your invoicing on.',
      },
      {
        slug: 'paypal',
        name: 'PayPal',
    ...
    faqs: [
      ...
      },
      {
        ...
        a:
          'Honestly, we cannot rank this one as cleanly as we would like, and it is better to say so than to pretend. The three options most likely to suit a freelancer are the three we cannot price. Higlobe, GrabrFi and Revolut Mexico each publish no exchange rate margin, and GrabrFi\'s withdrawal fee comes back three different ways depending on which source you read, so none of them appears in the comparison above. On a $5,000 invoice a spread you cannot see will dwarf a fee you can, and that is why we will not put a number on them. Of what we can price, Payoneer is the most predictable and roughly 3% all in, which is the price of not having to think about it. For a single large payment a wire is competitive once the flat fees stop mattering, and it gives you the cleanest documentation. Our practical suggestion: open accounts at two of the unpriced three, get a real quote for your actual invoice size on the same afternoon, compare the pesos that land rather than the fees quoted, and factor in what your accountant says about the IVA question above.',
      },
    ],
